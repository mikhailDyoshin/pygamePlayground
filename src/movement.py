...
from pygame import K_a, K_d, K_s, K_w, Vector2
...
def get_velocity(v0: Vector2, dt: float, pressed_keys: ScancodeWrapper) -> Vector2:
    ...
    if acc.length() == 0 and v0.length() < STOP_EPSILON:
        return Vector2(0, 0)
...
def get_new_position(
    *,
    ...
    return get_distance(current_position, get_velocity(current_velocity, dt, keys), dt)
