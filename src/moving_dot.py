...
def screen_wrap(position: Vector2, width: float, height: float) -> Vector2:
    ...
    if position.y < 0:
        return Vector2(position.x, height)
...
def random_vector(max_projection: int) -> Vector2:
    ...
    return Vector2(x_projection * randint(-1, 1), y_projection * randint(-1, 1))
...
def update_shape_position(*, position: Vector2, shape: ShapeData) -> ShapeData:
